...
const mockApplications = [
    ...
    { name: "Make Payment", icon: FaThLarge, link: "/payment" },
];
        ...
        <div className={`relative ${className}`}>
            ...
            {isOpen && (
                <>
                    ...
                    <div className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-2xl border border-gray-100 z-50 transform origin-top-right animate-fade-in-down">
                        {children}
                    </div>
                </>
            )}
...
// Notifications Menu
const NotificationsMenu = ({ notifications, navigate }) => (
    <>
    ...
    return (
        <div className="sticky top-0 bg-white shadow-lg z-30">
            <header className="flex justify-between items-center p-4 border-b border-gray-100">
                <button onClick={toggleSidebar} className="md:hidden p-2 rounded-lg hover:bg-gray-100 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <FaBars className="text-xl text-gray-700" />
                </button>
                
                <h1 className="text-lg md:text-2xl font-extrabold text-gray-900 mx-auto md:mx-0">SkillLinker</h1>
                
                <div className="flex items-center space-x-2 md:space-x-4">
                    <DropdownMenu trigger={<FaThLarge className="text-xl text-gray-600" />} className="hidden sm:block">
                    ...
                    <DropdownMenu trigger={
                        ...
                        <NotificationsMenu notifications={mockNotifications} navigate={navigate} />
                    </DropdownMenu>
                    ...
                    <DropdownMenu trigger={
                        <div className="flex items-center space-x-2 cursor-pointer p-1 rounded-full bg-blue-50 hover:bg-blue-100 transition-colors">
                            ...
                            <span className="text-sm font-medium hidden sm:inline text-gray-800">{currentUser.firstName}</span>
            ...
            <nav className="flex justify-around md:justify-start md:space-x-8 px-4 py-2 border-t border-gray-100 bg-gray-50/50">
                <button onClick={() => setActiveTab("profile")} className={`flex items-center text-sm md:text-base py-2 transition-colors border-b-2 ${activeTab === "profile" ? "text-blue-600 border-blue-600 font-extrabold" : "text-gray-600 border-transparent hover:text-blue-500"}`}>
                    <FaUser className="inline mr-1 text-base md:text-lg" /> Profile
                </button>
                <button onClick={() => setActiveTab("jobs")} className={`flex items-center text-sm md:text-base py-2 transition-colors border-b-2 ${activeTab === "jobs" ? "text-blue-600 border-blue-600 font-extrabold" : "text-gray-600 border-transparent hover:text-blue-500"}`}>
...
const ModeratorAssessorDashboard = () => {
    ...
    if (!currentUser) return <div className="text-center py-12 text-gray-500">Loading user...</div>;
    ...
    const filteredJobs = useMemo(() => {
        ...
        return jobs.filter((job) =>
            ...
            String(job.id).includes(lowerCaseFilter) ||
    ...
    const renderSidebarJobList = (jobList) => {
        if (loadingJobs) return <div className="text-gray-500 p-2">Loading...</div>;
        if (jobList.length === 0) return <div className="text-gray-400 p-2 text-sm">No jobs match your filter.</div>;
                ...
                {jobList.map((job) => (
                    ...
                    >
                        <span className="font-medium text-sm text-gray-700 truncate">
                            {job.title} <span className="text-xs text-gray-400 ml-1">(ID: {job.id})</span>
                        </span>
                        <span className={`px-2 py-0.5 text-xs font-semibold rounded-full flex-shrink-0 ${job.status === "open" ? "bg-green-100 text-green-800" : job.status === "in-progress" ? "bg-blue-100 text-blue-800" : "bg-gray-100 text-gray-800"}`}>
                            {job.status}
                        </span>
                    </li>
                ))}
    ...
    return (
        <div className="flex flex-col min-h-screen bg-gray-100">
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                {activeTab === "jobs" && (
                    <aside className={`fixed inset-y-0 left-0 w-64 bg-white shadow-2xl p-4 z-40 transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:shadow-lg md:w-64 md:h-auto md:overflow-y-auto ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                        <h2 className="text-xl font-extrabold mb-4 flex items-center text-blue-700 border-b pb-2"><FaBriefcase className="mr-2" /> All Job Listings</h2>
                        {renderFilterInput()}
                ...
                <main className="flex-1 overflow-y-auto">
                    <div className="p-4 sm:p-6 bg-gray-100 min-h-full">
                        ...
                        {activeTab === "jobs" && (
                            <>
                                <h2 className="text-2xl font-extrabold mb-6 text-gray-800 border-b pb-3">Available Jobs</h2>
                                <div className="block md:hidden mb-6">{renderFilterInput()}</div>
                                ...
                                <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                                    {loadingJobs ? (
                                        ...
                                        filteredJobs.map((job) => (
                                            <div key={job.id} className="shadow-xl rounded-xl bg-white overflow-hidden hover:shadow-2xl hover:scale-[1.02] transition-all duration-300 cursor-pointer border border-gray-100" onClick={() => navigate(`/job-details/${job.id}`)}>
                                                <div className="p-5 flex flex-col justify-between h-full">
                                                    <div>
                                                        <div className="flex justify-between items-start mb-2">
                                                            <h3 className="text-lg font-extrabold text-blue-700 line-clamp-2">{job.title}</h3>
                                                            <span className={`ml-3 px-3 py-1 text-xs font-bold rounded-full flex-shrink-0 ${job.status === "open" ? "bg-green-100 text-green-800" : job.status === "in-progress" ? "bg-blue-100 text-blue-800" : "bg-gray-100 text-gray-800"}`}>
                                                                {job.status}
                                                            </span>
                                                        </div>
                                                        ...
                                                        <div className="flex items-center mt-2 mb-2">
                                                            {[...Array(5)].map((_, i) => (
                                                                <FaStar key={i} className={`mr-1 text-sm ${i < (job.rating || 0) ? "text-yellow-400" : "text-gray-300"}`} />
                                                            ))}
                                                            <span className="text-xs text-gray-500 ml-2">{job.rating ? `${job.rating}/5` : "No rating"}</span>
                                                        </div>

                                                        <p className="text-xs font-medium text-gray-500 mb-4">Job ID: {job.id} | SDP ID: {job.sdpId}</p>
                                                        <p className="mt-2 text-sm text-gray-600 line-clamp-3 mb-4">{job.description}</p>

                                                        <div className="text-sm space-y-2 text-gray-500 border-t pt-4">
                                                            <div className="flex items-center"><FaMapMarkerAlt className="mr-2 text-red-400" /> <span className="truncate">{job.location}</span></div>
                                                            <div className="flex items-center"><FaCalendarAlt className="mr-2 text-yellow-500" /> Deadline: {job.deadline ? new Date(job.deadline).toLocaleDateString() : "N/A"}</div>
                                                        </div>
                                                    </div>

                                                    <div className="mt-6 text-xl font-extrabold text-green-600 border-t pt-4">
                                                        {job.budget ? `R${parseFloat(job.budget).toLocaleString("en-ZA", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : "N/A"}
                                                    </div>
                                                </div>
                                            </div>
                                        ))
